...
@section('content-header')
    ...
    <div class="max-w-4xl mx-auto flex justify-end mb-2">
        <a href="{{ route('sdm.rekammedis.pdf', $rekamMedis->id) }}" target="_blank"
...
@section('content')
    <div class="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div><strong>Nama Pasien:</strong><br>{{ $rekamMedis->kunjungan->pasien->user->name ?? '-' }}</div>
            <div><strong>Keluhan:</strong><br>{{ $rekamMedis->kunjungan->keluhan ?? '-' }}</div>
            <div><strong>Tanggal
                    Kunjungan:</strong><br>{{ \Carbon\Carbon::parse($rekamMedis->kunjungan->tgl_kunjungan)->format('d-m-Y H:i') }}
            </div>
            <div><strong>Tanggal
                    Ditangani:</strong><br>{{ \Carbon\Carbon::parse($rekamMedis->created_at)->format('d-m-Y H:i') }}
            </div>
        </div>
