...
@section('content-header')
    {{-- Tombol PDF --}}
    <div class="max-w-4xl mx-auto flex justify-end p-4">
...
@section('content')
    ...
    <div class="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div><strong>Nama Pasien:</strong><br>{{ $kunjungan->pasien->user->name ?? '-' }}</div>
            <div><strong>NID:</strong><br>{{ $kunjungan->pasien->user->nid ?? '-' }}</div>

            <div><strong>Tanggal
                    Kunjungan:</strong><br>{{ \Carbon\Carbon::parse($kunjungan->tgl_kunjungan)->setTimezone('Asia/Jakarta') }}
            </div>
            <div>
            ...
            <div class="sm:col-span-2"><strong>Keluhan:</strong><br>{{ $kunjungan->keluhan ?? '-' }}</div>
        </div>
        ...
        @if ($kunjungan->rekamMedis)
            <div class="mb-4">
                ...
                @if (empty($kunjungan->rekamMedis->diagnosis) || empty($kunjungan->rekamMedis->tindakan))
